...
#[tokio::test]
#[ignore]
async fn online_rewrite_big_table_bigint_pk() {
    init_tracing();
    ...
    let writer_rows_total = env_u64("ORSX_BIG_WRITER_ROWS", 0);
    let writer_batch = env_u64("ORSX_BIG_WRITER_BATCH", 50_000);

    sqlx::query("DROP TABLE IF EXISTS orsx2_big_bigint CASCADE")
        .execute(&pool)
        .await
        .unwrap();
    ...
    let insert_task = tokio::spawn({
        ...
        async move {
            if writer_rows_total == 0 {
                return 0u64;
            }
            let mut inserted_total: u64 = 0;
            let mut next_id: i64 = rows as i64 + 1;
            while inserted_total < writer_rows_total {
                let batch = std::cmp::min(writer_batch, writer_rows_total - inserted_total);
                let res = sqlx::query(
                    r#"
                    INSERT INTO orsx2_big_bigint (id, c01,c02,c03,c04,c05,c06,c07,c08,c09,c10,
                      c11,c12,c13,c14,c15,c16,c17,c18,c19,c20,
                      c21,c22,c23,c24,c25,c26,c27,c28,c29,c30,
                      c31,c32,c33,c34,c35,c36,c37,c38,c39,c40,
                      c41,c42,c43,c44,c45,c46,c47,c48,c49)
                    SELECT
                      (gs + $1::bigint - 1)::bigint,
                      (gs % 2147483000)::int4,
                      ((gs + 1) % 2147483000)::int4,
                      ((gs + 2) % 2147483000)::int4,
                      ((gs + 3) % 2147483000)::int4,
                ...
                match res {
                    Ok(done) => {
                        inserted_total = inserted_total.saturating_add(done.rows_affected() as u64);
                        next_id = next_id.saturating_add(done.rows_affected() as i64);
                    }
                    Err(e) => {
                        eprintln!("insert_task: insert query failed: {e}");
                        break;
                    }
                }
            }
    ...
    let cfg = MigrationConfig {
        ...
        synchronous_commit_off: env_bool("ORSX_SYNC_COMMIT_OFF", false),
        parallel_backfill: env_bool("ORSX_PARALLEL_BACKFILL", false),
        parallel_backfill_workers: env_u64("ORSX_PARALLEL_WORKERS", 4) as usize,
        ..MigrationConfig::default()
    };

    println!(
        "cfg: sync_commit_off={} parallel_backfill={} workers={}",
        cfg.synchronous_commit_off, cfg.parallel_backfill, cfg.parallel_backfill_workers
    );
    ...
    println!("writer summary: inserted={inserted}");

    let nulls: (i64,) =
        sqlx::query_as("SELECT COUNT(*)::BIGINT FROM orsx2_big_bigint WHERE new_col IS NULL")
            .fetch_one(&pool)
            .await
            .unwrap();
    assert_eq!(nulls.0, 0);
